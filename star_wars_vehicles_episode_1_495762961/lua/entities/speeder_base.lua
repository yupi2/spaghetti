...
function ENT:Use(p)
	...
	if(not self.Inflight) then
		if(p:KeyDown(IN_WALK)) then
			...
			if table.HasValue(allowed_jobs,team.GetName(p:Team())) or p:IsAdmin() then
				print(team.GetName(p:Team()))
				self:Enter(p,true);
			else
				p:PrintMessage( HUD_PRINTTALK, "You are not a Clone Pilot" )
			end
		end
	else
