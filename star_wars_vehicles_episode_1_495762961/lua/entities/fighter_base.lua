...
function ENT:PunchingIt(Dest)
	if(!self.PunchIt) then
		...
		if(self.LightSpeedWarp < CurTime()) then
			...
			if EventTimer.Minigame and (self:GetNWEntity( "ShipOwner" ) != nil) then
			else
				local fx = EffectData()
		...
		if(IsValid(self)) then
			if(self.IsFPV and ShouldFPV) then
                ...
                if(self.Lightspeed != 2) then
					...
					-- Jacobs Edit --
					if EventTimer.Minigame then
						if (self:GetNWEntity( "ShipOwner" ) != nil) then
							for k,v in pairs( ents.GetAll() ) do
								if table.HasValue( FindInBox2(), v ) then
									if v != self:GetNWEntity( "ShipOwner" ) and v != LocalPlayer() then
